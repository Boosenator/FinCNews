...
interface PersonaRun {
  ...
  created_at: string;
  data_snapshot: { source?: string; article_category?: string } | null;
}
              ...
              <div key={run.id} className="flex items-start justify-between gap-4 py-2.5">
                <div className="min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {run.primary_signal?.startsWith("rss:") && (
                      <span className="rounded bg-sky-500/15 px-1.5 py-0.5 text-[9px] font-bold uppercase text-sky-400">RSS</span>
                    )}
                  </div>
                  <p className="mt-0.5 truncate text-[11px] text-zinc-600">{run.reasoning}</p>
                  {run.article_slug && (
                    <a
                      href={`/${run.data_snapshot?.article_category ?? "crypto"}/${run.article_slug}`}
                      target="_blank"
